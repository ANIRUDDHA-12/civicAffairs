...
const Header = () => {
  ...
  className={`nav-link block ${
    pathname === menu.url ? "text-red-500" : ""
  } hover:text-red-500 focus:text-red-500 active:text-red-500`} // Change text color to red on hover
>
  {menu.name}
</Link>
                  </li>
        ...
        {enable && (
          <div className="d-flex order-1 ml-auto hidden min-w-[200px] items-center justify-end md:order-2 md:ml-0 md:flex">
            <Link className="z-0 py-[14px] bg-red-400 text-white px-6 rounded-3xl" href={link} rel="">
              {label}
            </Link>
          </div>
        )}
